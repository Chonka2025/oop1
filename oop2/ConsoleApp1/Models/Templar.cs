...
namespace CharacterBattle
{
  public class Templar : MeleeCharacter
  {
    ...
    public override void Attack(Character target)
    {
      if (IsImmobilized)
      {
        BattleNarrator.Line($"  • {Name}: ход пропущен (обездвиживание)");
        return;
      }
      ...
      if (HolyPower >= 3)
      {
        ...
        damage += bonus;
        BattleNarrator.Line($"  • {Name}: удар с тратой святой силы (×{HolyPower}) → +{bonus} к урону, всего {damage}");
        HolyPower = 0;
      }
      else
      {
        HolyPower++;
        BattleNarrator.Line($"  • {Name}: обычный удар, урон {damage}; святая сила накоплена: {HolyPower}/3");
      }

      target.TakeDamage(damage);
      int faithBefore = Faith;
      Faith += 5;
      BattleNarrator.Line($"    вера: {faithBefore} → {Faith} (+5)");
    }
    ...
    public override void TakeDamage(int damage)
    {
      ...
      {
        BattleNarrator.Line($"  ← {Name}: урон не проходит (обездвиживание / эгида)");
        return;
      }
      ...
      BattleNarrator.Line($"  ← {Name}: броня {Armor} (поглощает {mitigated}), урон {raw} → {damage}");
    ...
    public void ActivateDivineAegis()
    {
      if (Faith >= 50 && !IsImmobilized)
      {
        ...
        Faith -= 50;
        BattleNarrator.Line($"  • {Name}: использует неуязвимый купол! (осталось: {Faith} вера)");
      }
